import type {FlowDependencyGraph, FlowDependencies} from "@circlesland/interfaces-flow-runner";
...
export class FlowDependencySearch {
   ...
   * @param flowId
   */
  static findDependencies(flowRepository: IFlowRepository, flowId: string) : FlowDependencyGraph {
    ...
    while (dependencyScanBacklog.length > 0) {
      ...
      const currentDependencyManifest = flowRepository.tryGet(currentDependency);
